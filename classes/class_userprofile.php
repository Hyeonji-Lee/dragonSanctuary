...
class UserProfile extends Model{
  ...
  public function getFavpet(){
      if(is_numeric($this->favpet)){
          $this->favpet = ($this->favpet == 0)?new Comment("선택되지 않음"):new Link("levelup/click/{$this->favpet}", new Image("levelup/siggy/{$this->favpet}"), TRUE);   //"None Selected"
      }
      return $this->favpet;	  
  }
  ...
  private function aboutme(){
      ...
      $basicinfo = "<br><strong>가입일 :</strong> {$membersince}<br>
				    성별 : {$this->gender}<br>
				    좋아하는 색 : {$this->color}<br>
				    닉네임 : {$this->nickname}<br>
				    자기소개 : {$this->bio}";
        //<br><strong>Member Since:</strong> {$membersince}<br>
  ...
  private function showalladopts(){
      $mysidia = Registry::get("mysidia");	
	  $document = $mysidia->frame->getDocument();
      $spotlight = new Comment(".:대표 드래곤:.");   //".:AdoptSpotlight:."
      ...
      $title = new Comment("{$mysidia->input->get("user")}님의 드래곤들");   //"{$mysidia->input->get("user")}'s Pets:"
      $title->setBold(TRUE);
      $title->setUnderlined(TRUE);
      $document->add($title);
