...
class AdoptTableHelper extends TableHelper{
    ...
    public function getGenderImage($gender){
        if($adopt->currentlevel <= 2)    //레벨이 알일때 성별을 ?로 출력
        {
            return new Image("picuploads/u.png");
        }
        else    return new Image("picuploads/{$gender}.png");			
    }
	...
	public function getBasicInfo($name, $cost){
		...
		$nameField->setBold();
		$costField = new Comment("분양가: {$cost}", FALSE);
	...
	public function getAdditionalInfo($adopt){
	    $info = new ArrayList;
	    $info->add(new Comment("레벨: {$adopt->getCurrentLevel()} "));
		$info->add(new Comment("성별: ", FALSE));
    ...
    public function __toString(){
	    return new String("This is an instance of Mysidia AdoptTableHelper class.");
	}    
} 
?>