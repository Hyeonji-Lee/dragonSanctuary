...
class OwnedAdoptable extends Adoptable{
	...
	public function getGender($fetchMode = ""){
        if(($fetchMode == Model::GUI) && ($this->currentlevel <= 2))
        {
            return new Image("picuploads/u.png");    //레벨이 알일때 성별을 ?로 출력
        }
	    elseif($fetchMode == Model::GUI) return new Image("picuploads/{$this->gender}.png");    //해츨링 이상이면 성별 출력
	...
	public function getStats(){
		...
		$stats->add(new Comment("<br><br><b>클릭 수: {$this->totalclicks}"));
		$stats->add(new Comment("성별: ", FALSE));
        if($this->currentlevel <= 2)    //레벨이 알일때 성별을 ?로 출력
        {
            $stats->add(new Image("picuploads/u.png"));
        }
        else
        {
       ...
       /* $stats->add(new Comment("<br><br><b>Total Clicks: {$this->totalclicks}"));
		...
		$adoptStats = "<br>현재레벨: {$this->currentlevel}<br>다음레벨: {$nextLevel}</b>";
