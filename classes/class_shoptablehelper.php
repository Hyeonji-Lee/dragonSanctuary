...
class ShopTableHelper extends TableHelper{
    ...
    public function getShopStatus($shop){
        if($shop->status == "open")
        {
            if($shop->shopname=="test and debug"){
                return new Link("./shop/browse/{$shop->shopname}","<h3><strong>{$shop->shopname}</strong></h3>");
            }
            else{
                return new Link("shop/browse/{$shop->shopname}", new Image($shop->imageurl));
            }
        }//상점 이름, 링크
